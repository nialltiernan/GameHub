    ...
    <div class="relative inline-block">
        <a href="{{ route('game.show',['id' => $game['id']]) }}">
            <img src="{{ $game['image_url'] }}" alt="game cover"
                 class="hover:opacity-75 transition ease-in-out duration-150" style="max-height: 215px">
        </a>

        @if (isset($game['slug']))
            <div class="absolute bottom-0 right-0 w-16 h-16 bg-gray-800 rounded-full" style="right:-20px; bottom:-20px">
                <div id="{{ $game['slug'] }}"
                     class="font-semibold text-xs flex justify-center items-center h-full"></div>
            </div>
        @endif
    </div>
